...
def f (plainText):
    ...
    for i in range(0,8,2):
        col = int(hex2bin(plainText[i:i+2]),2)
        a[int(i/2)] = S[int(i/2)][col]
        
    ans = addBin(a[0], a[1])
    ans = bin2hex(xor(hex2bin(ans), hex2bin(a[2])))
    ans = addBin(ans, a[3])
    
    return ans
    
def iteration(time,plainText):
...
def encrypt(plainText):
    ...
    R = plainText[0:8]
    L = plainText[8:16]
    R = xor(hex2bin(R), P[16])
    L = xor(hex2bin(L), P[17])
    R = bin2hex(R)
    L = bin2hex(L)
    return L+R;
